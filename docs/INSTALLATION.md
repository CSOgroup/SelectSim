...
`micromamba` is a tiny version of the mamba package manager (Like
Conda). Refer
[website](https://mamba.readthedocs.io/en/latest/installation/micromamba-installation.html)
for it’s installation guide.
...
Alternative way install with provided
[`enviorment.yml`](https://csogroup.github.io/SelectSim/enviorment.yml)
file.

`micromamba create -f enviorment.yml`\
`micromamba activate r_env` - After this run R and install `SelectSim` R
package as follows
